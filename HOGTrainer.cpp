...
void HOGTrainer::getSVMDetector(const Ptr<SVM> &svm, vector<float> &hog_detector) {
...
void HOGTrainer::convertToMl(const std::vector<cv::Mat> &train_samples, cv::Mat &trainData) {
    //--Convert data
...
void HOGTrainer::samplePos(vector<Mat> &fullPosLst, vector<Mat> &posLst, Size &size) {
    Rect box;
    ...
    vector<Mat>::const_iterator img = fullPosLst.begin();
    vector<Mat>::const_iterator end = fullPosLst.end();
    for (; img != end; ++img) {
        if(img->cols >= box.width && img->rows >= box.height) {
            box.x = (img->cols - size_x) == 0 ? 0 : (img->cols - size_x)/2;
            box.y = (img->rows - size_y) == 0 ? 0 : (img->rows - size_y)/2;;
            Mat roi = (*img)(box);
            posLst.push_back(roi.clone());
...
void HOGTrainer::sampleNeg(const vector<Mat> &full_neg_lst, vector<Mat> &neg_lst, const Size &size) {
    Rect box;
    box.width = size.width;
    box.height = size.height;
    ...
    for (; img != end; ++img) {
        box.x = (img->cols - size_x) == 0 ? 0 : rand() % (img->cols - size_x);
        box.y = (img->rows - size_y) == 0 ? 0 : rand() % (img->rows - size_y);
        Mat roi = (*img)(box);
...
Mat HOGTrainer::getHogdescriptorVisu(const Mat &color_origImg, vector<float> &descriptorValues, const Size &size) {
    ...
    float radRangeForOneBin = (float) (CV_PI /
                                       (float) gradientBinSize); // dividing 180 into 9 bins, how large (in rad) is one bin?
        ...
        for (cellx = 0; cellx < cells_in_x_dir; cellx++) {
            ...
            for (int bin = 0; bin < gradientBinSize; bin++) {
                ...
                line(visu, Point((int) (x1 * zoomFac), (int) (y1 * zoomFac)),
                     Point((int) (x2 * zoomFac), (int) (y2 * zoomFac)), Scalar(0, 255, 0), 1);
...
void HOGTrainer::trainSvm(const vector<Mat> &gradient_lst, const vector<int> &labels) {
...
void HOGTrainer::drawLocations(Mat &img, const vector<Rect> &locations, const Scalar &color) {
    if (!locations.empty()) {
